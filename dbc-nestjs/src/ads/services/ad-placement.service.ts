...
export class AdPlacementService {
  ...
  public async createAd(Ad: Omit<Ad, '_id'>) {
    return await this.dbProbider.ads().create(Ad);
  }
  ...
  public async updateAd(id: number, ad: Partial<Ad>) {
    return await this.dbProbider
      .ads()
      .findByIdAndUpdate(id, { $set: ad }, { new: true })
      .lean();
  }

  public async deleteAd(id: number) {
    return await this.dbProbider.ads().findByIdAndDelete(id).lean();
  }
}
